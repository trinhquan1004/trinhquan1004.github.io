...
class World {
  ...
  addHeart() {
    ...
    const colorChoices = [
      ...
      "green",
      "LightPink",
      "Orchid1","Orange1","LightSkyBlue", "LightYellow"
    ];
  ...
  async addModel() {
    this.model = await this.loadObj(
      ...
      //File GLB là định dạng tệp nhị phân, còn được gọi là JPEG cho 3D, mô tả cảnh 3D cho định dạng web.
    );
  ...
  loadMusic() {
    return new Promise((resolve) => {
      ...
      audioLoader.load(
        ...
        "./sound/FallingYou_Lighter_and_Princess.mp3",
        (buffer) => {
  ...
  addSnow() {
    ...
    const randoms = new Float32Array(count);
    const randoms1 = new Float32Array(count);
    const colorChoices = ["red", "pink", "hotpink", "green","Orange1","LightSkyBlue","Yellow1"];
